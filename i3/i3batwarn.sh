...
if [ $PERCENT -le "15" ] && [ "$STAT" == "Discharging" ]; then
    DISPLAY=:0.0 /usr/bin/i3-nagbar -m "Bateria!!!"
fi

if [ $PERCENT -le "5" ] && [ "$STAT" == "Discharging" ]; then
    DISPLAY=:0.0 /usr/bin/i3-nagbar -m "BATERIA CARA!!! BATERIA!!!"
fi
